/*
 * ============================================================
 * EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.32
 * AVOID-ONLY TEST: detect any object → avoid → rejoin line
 * No color check, no pick, no carry, no drop.
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.32 — AVOID-ONLY TEST");
  Serial.println("============================================");
...
void loop() {
  ...
  if (isAutoMode) {
    ...
    switch (currentState) {
      ...
      // ── LINE FOLLOW: detect anything → avoid immediately ───────
      case STATE_LINE_FOLLOW: {
        ...
        if (dist > 2 && dist < DETECT_DIST_CM) {
          ...
          Serial.print(dist, 1);
          Serial.println("cm — avoiding immediately");
      ...
      // ── AVOID: reverse, bypass, drive past, turn back ──────────
      ...
      // ── REJOIN: arc search to find line again ──────────────────
      case STATE_REJOIN_LINE: {
        ...
        if (found) {
          ...
          Serial.println("Rejoin timeout — forward + spin retry");
      ...
      // ── Unused states — kept for enum safety, redirect to LINE_FOLLOW ──
      case STATE_IDENTIFY_OBJECT:
      case STATE_PICK_GREEN:
      case STATE_CARRY_TO_END:
      case STATE_DROP_CUBE: {
        Serial.println("Unused state reached — redirecting to LINE_FOLLOW");
        changeState(STATE_LINE_FOLLOW);
        break;
      }
...
// ============================================================
//  END OF CODE v7.32 — EC6090 Mini Project 2026
//  AVOID-ONLY TEST: Line follow → detect → avoid → rejoin.
//  No color, no pick, no carry, no drop.
//  Loop: LINE_FOLLOW → AVOID_RED → REJOIN_LINE → LINE_FOLLOW
// ============================================================
