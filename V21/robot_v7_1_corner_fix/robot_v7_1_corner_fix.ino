/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.1
 *  FIX — MISSED CORNER RECOVERY
 * ============================================================
 ...
 * ============================================================
 *
 *  ROOT CAUSE OF BUG (v7.0):
 *    When the robot approaches a 90° or sharp 45° corner at
 *    speed, the sensor array physically passes over the corner
 *    tip. The line jumps from one side (e.g. 01000) straight
 *    to ALL-OFF (00000) — skipping the 10000 pattern that was
 *    supposed to trigger the pivot.
 *
 *    The old code then hit the lost-line recovery block which
 *    spun the robot in a small arc instead of committing to a
 *    full corner pivot. The robot would miss the turn entirely
 *    or make a weak partial turn and drive off the track.
 *
 *  FIX 1 — MISSED CORNER DETECTION:
 *    Before entering the normal lost-line recovery, check
 *    whether the last PID error was strongly to one side
 *    (|lastError| >= MISSED_CORNER_THRESHOLD).
 *    If it was, AND all sensors just went dark → the robot
 *    flew past the corner tip without catching 10000/00001.
 *    Commit immediately to a pivot in the direction of the
 *    last strong error — exactly as if 10000/00001 had fired.
 *
 *    Threshold = 30 (out of ±40 max weight range).
 *    This means only a genuinely off-center last reading
 *    triggers the recovery — not a minor wobble on a straight.
 *
 *  FIX 2 — PIVOT NEVER INTERRUPTED BY SENSOR-SUM CHECK:
 *    The pivot loop already had a `return` before the PID
 *    section — but the lost-line block is INSIDE the PID
 *    section. Since `turning==true` causes an early `return`,
 *    the lost-line block was never reached during a pivot.
 *    However, a hard timeout has been added (3000ms) so the
 *    robot cannot pivot forever if something goes wrong.
 *
 *  FIX 3 — PIVOT EXIT WIDENED:
 *    Exit condition changed from (S2||S3) to (S2||S3||S4).
 *    After a missed corner recovery, the robot may reacquire
 *    the line slightly off-center. Accepting S4 as an exit
 *    prevents the robot from over-rotating past the line.
 *
 *  FIX 4 — CORNER ENTRY SPEED REDUCED:
 *    BASE_SPEED reduced 170→150. This gives sensors more time
 *    per frame to catch the 10000/00001 pattern before the
 *    array passes the corner tip. On straights the robot still
 *    accelerates to MAX_SPEED (200) via adaptive speed.
 *    So straight-line speed is unchanged; only curve entry is
 *    slightly slower, which is the correct trade-off.
 *
 ...
 *
 *  Author: EC6090 Mini Project 2026 — v7.1 Missed Corner Fix
 * ============================================================
 */
...
#define BASE_SPEED       150   // ← REDUCED from 170 (Fix 4: slower corner entry)
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.1 — Missed Corner Fix");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  if (!turning) {
    ...
    // Normal tight turn: far sensor fires, middle+center all off
    bool tightLeft  = (s1 && !s2 && !s3);
    bool tightRight = (s5 && !s4 && !s3);

    // ── FIX 1: MISSED CORNER DETECTION ──────────────────────────
    // Fires when ALL sensors go dark AND last error was strongly
    // off-center — robot flew past the corner tip at speed.
    int sensorSumCheck = irBinary[0]+irBinary[1]+irBinary[2]
                        +irBinary[3]+irBinary[4];

    bool missedCornerLeft  = (sensorSumCheck == 0 &&
                               lastError <= -MISSED_CORNER_THRESHOLD);
    bool missedCornerRight = (sensorSumCheck == 0 &&
                               lastError >=  MISSED_CORNER_THRESHOLD);

    if (tightLeft || missedCornerLeft) {
      if (missedCornerLeft) {
        Serial.println("MISSED CORNER LEFT → recovering (lastError was strongly left)");
      } else {
    ...
    else if (tightRight || missedCornerRight) {
      if (missedCornerRight) {
        Serial.println("MISSED CORNER RIGHT → recovering (lastError was strongly right)");
      } else {
  ...
  // ── TURNING MODE ─────────────────────────────────────────────
  // NOTE: once turning==true, this block returns immediately.
  // The sensorSum==0 lost-line block below is NEVER reached
  // during a pivot — the pivot handles all-dark by itself.
  if (turning) {
    ...
    // Guard: wait at least 500ms before checking exit condition
    // This prevents the robot from exiting the pivot immediately
    // on the same sensor reading that triggered it.
    if (millis() - turnStartTime > 500) {
      readIRSensors();

      // ── FIX 3: WIDENED EXIT CONDITION ───────────────────────
      // Accept S2, S3, or S4 as reacquire confirmation.
      // S4 added vs v7.0 — handles slight over-rotation on
      // missed-corner recoveries where robot comes back to line
      // at a small angle.
    ...
    // ── FIX 2: HARD PIVOT TIMEOUT ───────────────────────────────
    // If center never reacquires within PIVOT_TIMEOUT_MS,
    // force-exit the pivot so the robot doesn't spin forever.
    ...
    return;  // ← PID completely OFF while pivoting
             //   sensorSum==0 block below is never reached
  }
  ...
  // ── LOST-LINE RECOVERY ───────────────────────────────────────
  // This block is only reached when turning==false.
  // If turning==true the function already returned above.
  // So sensorSum==0 here means genuine line loss (gap in tape,
  // or robot drifted off) — NOT a corner tip flyover.
  // The corner tip flyover case is now caught by missedCornerLeft/
  // missedCornerRight above BEFORE reaching this block.
  if (sensorSum == 0) {
    if (!lineLostTiming) {
    ...
    if (millis() - lineLostStart < 200) {
      // Short loss — coast in last known direction
...
void printDashboard() {
  ...
  Serial.print(" turn:"); Serial.print(turning ? (turnDirection<0?"L":"R") : "-");
  float dist = readUltrasonic();
...
// ============================================================
//  END OF CODE v7.1 — EC6090 Mini Project 2026 — Corner Fix
// ============================================================
