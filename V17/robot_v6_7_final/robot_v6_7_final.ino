/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v6.7
 *  BUGFIX — CORRECT TIGHT TURN TRIGGER CONDITION
 * ============================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v6.7 — Correct Tight Turn Trigger");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  if (turning) {
    ...
    // Guard: wait at least 80ms before checking exit condition
    // Prevents re-detecting the same line segment we just left
    if (millis() - turnStartTime > 80) {
      readIRSensors();   // fresh read inside turning loop
      if (irBinary[2] == 1) {   // S3 center sees the new line
        stopMotors();
        delay(20);
        turning   = false;
        lastError = 0;
