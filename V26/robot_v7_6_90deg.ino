/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.6
 *  90-DEG PATTERN — adds 11100/00111 trigger alongside 10000/00001
 * ============================================================
 ...
 *
 *  PROBLEM IDENTIFIED:
 *    At a 90° corner the sensor array does NOT show 10000.
 *    Because the line is wide enough, when the array reaches
 *    the corner it covers S1+S2+S3 at the same time:
 *
 *    45° corner pattern:   10000  or  00001
 *    90° corner pattern:   11100  or  00111
 *
 *    v7.5 only triggered on 10000/00001 so 90° corners were
 *    not detected and the robot drove straight through them.
 *
 *  FIX — ADDED TWO NEW TRIGGER PATTERNS:
 *
 *    sharp90Left  = (s1 && s2 && s3 && !s4 && !s5) = 11100
 *    sharp90Right = (!s1 && !s2 && s3 && s4 && s5) = 00111
 *
 *    These are added alongside the existing 45° triggers.
 *    Both trigger the EXACT same pivot behaviour — stop,
 *    pivot until S3 center sees the line, stop, resume PID.
 *    Nothing else changed.
 *
 *  FULL TRIGGER TABLE (v7.6):
 *
 *    Pattern   Meaning              Action
 *    ───────────────────────────────────────────────
 *    10000     45° left corner      Pivot RIGHT
 *    11100     90° left corner      Pivot RIGHT
 *    00001     45° right corner     Pivot LEFT
 *    00111     90° right corner     Pivot LEFT
 *    00000*    Missed corner        Backup + Pivot
 *    (* only if |lastError| >= 30)
 *
 *  ALL v7.5 LOGIC KEPT EXACTLY:
 *    - Pivot exits ONLY when S3 center detects the line
 *    - 30ms guard time
 *    - Phase 1 early warning slowdown
 *    - Phase 3 controlled reacquire
 *    - Missed corner backup
 *    - Pivot hard timeout 3000ms
 *    - All pivot direction fixes from v7.4 (slower corner entry)
 *
 ...
 *
 *  Author: EC6090 Mini Project 2026 — v7.6 90-Deg Pattern
 * ============================================================
 */
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.6 — 90-Deg Pattern");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  if (!turning && !reacquiring) {
    ...
    bool sharp90Left  = (s1 && s2 && s3 && !s4 && !s5);  // 90° left:  11100
    bool sharp90Right = (!s1 && !s2 && s3 && s4 && s5);  // 90° right: 00111
    ...
    if (tightLeft || sharp90Left || missedLeft) {
      if (missedLeft) {
        ...
        Serial.println("90-DEG LEFT-SENSOR (11100) → pivot RIGHT");
        stopMotors();
      } else {
      ...
      if (missedRight) {
        ...
        Serial.println("90-DEG RIGHT-SENSOR (00111) → pivot LEFT");
        stopMotors();
      } else {
...
// ============================================================
//  END OF CODE v7.6 — EC6090 Mini Project 2026 — 90-Deg Pattern
// ============================================================
