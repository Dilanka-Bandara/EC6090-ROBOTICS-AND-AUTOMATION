/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.4
 *  PIVOT DIRECTION FIX — sensors physically mounted reversed
 * ============================================================
 ...
 *
 *  BUG REPORTED:
 *    Far Right sensor fires → robot turns LEFT (should be RIGHT)
 *    Far Left  sensor fires → robot turns RIGHT (should be LEFT)
 *    Curves and straights work correctly.
 *
 *  ROOT CAUSE — SENSORS PHYSICALLY MOUNTED REVERSED:
 *    The code labels irPins[0] as "Far Left" and irPins[4] as
 *    "Far Right". But on this robot the physical mounting order
 *    is reversed — what the code calls S1 (GPIO 35) is actually
 *    on the RIGHT side of the robot, and S5 (GPIO 32) is on
 *    the LEFT side.
 *
 *    WHY CURVES STILL WORKED:
 *    In PID mode, the weighted error formula produces a negative
 *    error when the line is to the left. The correction is then
 *    subtracted from left speed and added to right speed — which
 *    steers toward the line. If sensors are reversed, the error
 *    sign is also reversed, but the motor correction formula
 *    applies it consistently — so PID self-corrects regardless
 *    of physical sensor order. The curve following appears fine.
 *
 *    WHY PIVOTS FAILED:
 *    Pivot direction is hard-coded: tightLeft → turnDirection=-1
 *    (pivot left). This does NOT self-correct. If S1 is physically
 *    on the RIGHT, detecting it as "tight left" and pivoting LEFT
 *    drives the robot away from the line, not toward it.
 *
 *  THE FIX — TWO CHANGES:
 *
 *  1. PIVOT DIRECTION SWAPPED:
 *     tightLeft  trigger → turnDirection = +1 (pivot RIGHT)
 *     tightRight trigger → turnDirection = -1 (pivot LEFT)
 *     missedLeft         → turnDirection = +1 (pivot RIGHT)
 *     missedRight        → turnDirection = -1 (pivot LEFT)
 *
 *  2. lastTightDir SWAPPED to match:
 *     tightLeft  → lastTightDir = +1
 *     tightRight → lastTightDir = -1
 *     This keeps the lost-line spin recovery consistent with
 *     the corrected pivot direction.
 *
 *  3. EARLY WARNING PATTERN LABELS SWAPPED:
 *     earlyLeft  (s1+s2 firing) now correctly triggers a RIGHT
 *     approach slow-down (because s1 is physically right side).
 *     earlyRight (s4+s5 firing) triggers a LEFT approach.
 *     The cornerApproach slowdown behaviour is unchanged —
 *     only the conceptual label changes. Speed is the same.
 *
 *  ALL v7.3 FEATURES KEPT:
 *    Phase 1 — Early warning slowdown (CORNER_APPROACH_SPEED)
 *    Phase 2 — Full pivot (SHARP_TURN_SPEED)
 *    Phase 3 — Controlled reacquire (REACQUIRE_SPEED)
 *    Missed corner backup before pivot
 *    All v7.1 and v7.2 fixes (slower corner entry)
 *
 ...
 *
 *  Author: EC6090 Mini Project 2026 — v7.4 Pivot Direction Fix
 * ============================================================
 */
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.4 — Pivot Direction Fix");
  Serial.println("============================================");
  ...
  if (!turning && !reacquiring) {
    ...
    if (tightLeft || missedLeft) {
      if (missedLeft) {
        ...
        Serial.println("LEFT-SENSOR tight → pivot RIGHT");
        stopMotors();
      }
      turning        = true;
      turnDirection  = 1;    // ← FIXED: S1 is physically RIGHT side → pivot RIGHT
      lastTightDir   = 1;    // ← FIXED: consistent with corrected direction
      cornerApproach = false;
      integralSum    = 0.0;
      ...
      if (missedRight) {
        ...
        Serial.println("RIGHT-SENSOR tight → pivot LEFT");
        stopMotors();
      }
      turning        = true;
      turnDirection  = -1;   // ← FIXED: S5 is physically LEFT side → pivot LEFT
      lastTightDir   = -1;   // ← FIXED: consistent with corrected direction
      cornerApproach = false;
      integralSum    = 0.0;
      turnStartTime  = millis();
...
// ============================================================
//  END OF CODE v7.4 — EC6090 Mini Project 2026 — Pivot Direction Fix
// ============================================================
