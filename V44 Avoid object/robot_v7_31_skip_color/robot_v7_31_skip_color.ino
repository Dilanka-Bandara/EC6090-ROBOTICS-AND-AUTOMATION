/*
 * ============================================================
 * EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.31
 * SKIP COLOR — ANY OBJECT DETECTED → AVOID IMMEDIATELY
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.31 — Skip Color, Avoid All");
  Serial.println("============================================");
...
void loop() {
  ...
  if (isAutoMode) {
    ...
    switch (currentState) {
      ...
      // v7.31: ANY object detected → avoid immediately (no color check)
      case STATE_LINE_FOLLOW: {
        ...
        if (dist > 2 && dist < DETECT_DIST_CM) {
          ...
          Serial.print(dist, 1);
          Serial.println("cm — skipping color, avoiding immediately");
      ...
      // ── STATE_IDENTIFY_OBJECT kept but NOT used in v7.31 ───────
      // (retained so state enum and array indices stay valid)
      case STATE_IDENTIFY_OBJECT: {
        // v7.31: This state is bypassed. If somehow reached, redirect.
        Serial.println("IDENTIFY_OBJECT reached unexpectedly — redirecting to AVOID");
        ultrasonicEnabled = false;
        changeState(STATE_AVOID_RED);
        break;
      }
...
// ============================================================
//  END OF CODE v7.31 — EC6090 Mini Project 2026
//  Skip Color variant: any object → avoid immediately.
//  No color check, no creep approach. All other logic from
//  v7.30 intact (IR-based direction, arc rejoin, WiFi, servo).
// ============================================================
