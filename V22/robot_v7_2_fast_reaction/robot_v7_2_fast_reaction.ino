/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.2
 *  FIX — FAST REACTION TIME (reduced IR→motor latency)
 * ============================================================
 ...
 *
 *  PROBLEM:
 *    The delay between IR sensor reading and motor output was
 *    too high. Every unnecessary delay() in the control path
 *    adds latency — the robot keeps moving at full speed during
 *    that dead time, making corners worse and reactions sluggish.
 *
 *  ALL DELAYS REMOVED OR REDUCED IN CONTROL PATH:
 *
 *  1. stopMotors() before pivot: delay(50) → REMOVED (0ms)
 *     Mechanical momentum settles in <2ms. The 50ms was
 *     causing the robot to drift further past the corner tip
 *     before the pivot even started.
 *
 *  2. Pivot exit after stopMotors(): delay(20) → REMOVED (0ms)
 *     No reason to pause after stopping before resuming PID.
 *
 *  3. Pivot timeout exit: delay(50) → REMOVED (0ms)
 *     Same — unnecessary pause before PID resumes.
 *
 *  4. Pivot guard time: 500ms → 300ms
 *     The guard prevents false early exit from pivot.
 *     300ms is still long enough to prevent false exit but
 *     200ms faster per corner. On a course with multiple
 *     corners this adds up significantly.
 *
 *  5. Debug dashboard interval: 100ms → 20ms (50Hz)
 *     Faster serial output means you can see sensor changes
 *     in real time during testing. No impact on control loop.
 *
 *  6. Ultrasonic removed from dashboard:
 *     pulseIn() inside printDashboard() was blocking the main
 *     loop for up to 2ms every 100ms even during line following.
 *     Dashboard now prints cached last distance instead.
 *     Ultrasonic is only read in the state machine where needed.
 *
 *  7. Main loop sensor read moved BEFORE state switch:
 *     readIRSensors() was already first in loop() — confirmed
 *     correct. No extra reads needed inside followLineAdaptive.
 *     The readIRSensors() call inside the pivot exit check is
 *     kept because it needs a fresh reading inside the pivot.
 *
 *  INHERITED FROM v7.1 (all fixes kept):
 *    Fix 1 — Missed corner detection (lastError threshold)
 *    Fix 2 — Pivot hard timeout (3000ms)
 *    Fix 3 — Pivot exit widened (S2||S3||S4)
 *    Fix 4 — BASE_SPEED 150 (slower corner entry)
 *
 ...
 *
 *  Author: EC6090 Mini Project 2026 — v7.2 Fast Reaction
 * ============================================================
 */
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.2 — Fast Reaction");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  if (!turning) {
    ...
    if (tightLeft || missedCornerLeft) {
      ...
      integralSum   = 0.0;
      stopMotors();
      // delay(50) REMOVED — no dead time before pivot starts
  ...
  if (turning) {
    ...
    // Guard: 300ms minimum before checking exit (reduced from 500ms)
    // Still long enough to prevent false exit on entry sensor flash.
    if (millis() - turnStartTime > 300) {
      ...
      if (irBinary[1] == 1 || irBinary[2] == 1 || irBinary[3] == 1) {
        stopMotors();
        // delay(20) REMOVED — no dead time before PID resumes
        turning   = false;
        lastError = 0;
...
void printDashboard() {
  ...
  // Use cached distance — avoids blocking pulseIn() call in main loop
...
// ============================================================
//  END OF CODE v7.2 — EC6090 Mini Project 2026 — Fast Reaction
// ============================================================
