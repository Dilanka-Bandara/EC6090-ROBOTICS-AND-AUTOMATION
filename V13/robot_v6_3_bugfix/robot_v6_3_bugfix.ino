/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v6.3
 *  BUG FIX — HARD TURN DIRECTION + PATTERN OVERLAP FIXED
 * ============================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v6.3 — Bug Fix: Hard Turn Direction");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  // ── 90° HARD CORNER DETECTION ───────────────────────────────
  //
  //  hardLeft:  ONLY S1 fires (not S2,S3,S4,S5).
  //             Line is far to the LEFT → robot must pivot LEFT.
  //             Pivot LEFT  = LEFT wheel BACKWARD, RIGHT wheel FORWARD.
  //             driveMotors(LEFT, RIGHT) → driveMotors(-speed, +speed)  ✓
  //
  //  hardRight: ONLY S5 fires (not S1,S2,S3,S4).
  //             Line is far to the RIGHT → robot must pivot RIGHT.
  //             Pivot RIGHT = LEFT wheel FORWARD, RIGHT wheel BACKWARD.
  //             driveMotors(LEFT, RIGHT) → driveMotors(+speed, -speed)  ✓
  //
  //  BUG FIXED (v6.1→v6.2→v6.3):
  //    Old hardLeft pattern allowed s2 to be true, causing S1+S2 to
  //    trigger hardLeft AND applying wrong motor direction (was turning RIGHT).
  //    Now: hardLeft requires !s2 (pure S1-only), steepLeft handles S1+S2.
  //    Motor directions corrected: LEFT pivot = (-speed, +speed).

  bool hardLeft  = (s1 && !s2 && !s3 && !s4 && !s5);  // ONLY S1
  bool hardRight = (s5 && !s1 && !s2 && !s3 && !s4);  // ONLY S5

  if (hardLeft) {
    integralSum = 0.0;
    Serial.println("HARD LEFT PIVOT (90°)");
    // Pivot LEFT: left wheel BACKWARD, right wheel FORWARD
    driveMotors(-SHARP_TURN_SPEED, SHARP_TURN_SPEED);
    // Hold pivot until S2, S3, or S4 picks up the line
    ...
    lastError = -20;  // seed PID: bias left so it doesn't overshoot right
    return;
  }

  if (hardRight) {
    integralSum = 0.0;
    Serial.println("HARD RIGHT PIVOT (90°)");
    // Pivot RIGHT: left wheel FORWARD, right wheel BACKWARD
    driveMotors(SHARP_TURN_SPEED, -SHARP_TURN_SPEED);
    unsigned long cornerStart = millis();
    while (millis() - cornerStart < 800) {
    ...
    lastError = 20;   // seed PID: bias right so it doesn't overshoot left
    return;
  }

  // ── 45° STEEP CORNER ASSIST ─────────────────────────────────
  //  steepLeft:  S1+S2 active, nothing on right half → sharp left arc
  //              Left wheel BACKWARD (or slow), right wheel FORWARD fast
  //              driveMotors(-speed, CURVE_SPEED)  ✓
  //
  //  steepRight: S4+S5 active, nothing on left half → sharp right arc
  //              Left wheel FORWARD fast, right wheel BACKWARD (or slow)
  //              driveMotors(CURVE_SPEED, -speed)  ✓
  bool steepLeft  = (s1 && s2 && !s4 && !s5);
  bool steepRight = (s4 && s5 && !s1 && !s2);

  if (steepLeft) {
    integralSum = 0.0;
    Serial.println("STEEP LEFT ARC (45°)");
    // Left wheel slightly backward, right wheel forward → tight left arc
    driveMotors(-(SHARP_TURN_SPEED - 10), CURVE_SPEED);
    lastError = -30;
    return;
  }
  ...
  if (steepRight) {
    integralSum = 0.0;
    Serial.println("STEEP RIGHT ARC (45°)");
    // Left wheel forward, right wheel slightly backward → tight right arc
    driveMotors(CURVE_SPEED, -(SHARP_TURN_SPEED - 10));
    lastError = 30;
    return;
  }
