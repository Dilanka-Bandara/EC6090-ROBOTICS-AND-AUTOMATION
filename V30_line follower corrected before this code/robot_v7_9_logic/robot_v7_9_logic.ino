...
#define DETECT_DIST_CM       20   // first detection — start creeping toward object
...
void loop() {
  ...
  switch (currentState) {
    ...
    case STATE_IDENTIFY_OBJECT: {
      ...
      // If green already captured → skip color check, go straight to avoid
      if (cubePickedUp) {
        Serial.println("Cube already held — auto avoiding object");
        changeState(STATE_AVOID_RED);
        break;
      }

      // Creep forward until ultrasonic reads <= COLOR_READ_DIST_CM
      float dist = readUltrasonic();
      if (dist > COLOR_READ_DIST_CM) {
        driveMotors(CREEP_SPEED, CREEP_SPEED);
        break;  // keep creeping, come back next loop
      }

      // Now at 5cm — stop and read color
      stopMotors();
      delay(200);
      ...
      if (color == 'G') {
        Serial.println("→ GREEN cube — grabbing");
        changeState(STATE_PICK_GREEN);
      } else {
        // RED or unknown → avoid
        Serial.println("→ RED/Unknown — reversing + avoiding");
        changeState(STATE_AVOID_RED);
      }
      break;
    }
