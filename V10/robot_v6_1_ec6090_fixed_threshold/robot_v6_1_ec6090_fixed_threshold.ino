 ...
 *
 *  1. FIXED THRESHOLD: Single IR_THRESHOLD value (2500) used for
 *     all sensors. Yellow tape on grey concrete typically reads
 *     ~3000-3800 (line) vs ~400-800 (floor). Threshold at 2500
 *     is safe midpoint. Adjust if needed after TEST_IR_RAW check.
 *     ► If your sensors read differently, just change IR_THRESHOLD.
 *
...
// --- IR Threshold ---
// Yellow tape on grey concrete: line reads ~3000-3800, floor reads ~400-800
// Threshold 2500 is a safe midpoint. Adjust after running TEST_IR_RAW mode.
// ► If your sensors read HIGH on floor and LOW on line, set INVERT_IR true
#define IR_THRESHOLD    2500
#define INVERT_IR       false   // set true if your sensor reads LOW on yellow
...
void setup() {
  ...
  Serial.print("INVERT_IR: ");    Serial.println(INVERT_IR ? "YES" : "NO");
...
void readIRSensors() {
  for (int i = 0; i < 5; i++) {
    ...
    if (INVERT_IR) {
      // Some sensors read LOW on yellow, HIGH on grey
      irBinary[i] = (irRaw[i] < IR_THRESHOLD) ? 1 : 0;
    } else {
      // Normal: yellow tape reflects MORE → higher reading
      irBinary[i] = (irRaw[i] > IR_THRESHOLD) ? 1 : 0;
    }
  }
}
