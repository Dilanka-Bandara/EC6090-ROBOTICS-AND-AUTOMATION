/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v6.6
 *  TIGHT TURN — FRIEND'S LOGIC ADAPTED (flag-based pivot)
 * ============================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v6.6 — Friend Logic Tight Turns");
  Serial.println("============================================");
...
// ============================================================
//  STABLE FAR SENSOR READ  (noise filter — same as friend's code)
//  Reads the far sensor 5 times, returns 1 (on line) only if
//  at least 4 of 5 readings confirm it. Prevents false triggers.
// ============================================================
int readFarSensorStable(int pin) {
  int onCount = 0;
  for (int i = 0; i < 5; i++) {
    int raw = digitalRead(pin);
    // Apply INVERT_IR same as readIRSensors()
    int val = INVERT_IR ? (raw == LOW ? 1 : 0) : (raw == HIGH ? 1 : 0);
    if (val == 1) onCount++;
    delayMicroseconds(800);
  }
  return (onCount >= 4) ? 1 : 0;  // confirmed ON only if 4/5 agree
}


// ============================================================
//  LINE FOLLOWING — PID + Tight Turn (v6.6)
//
//  TIGHT TURN LOGIC (from friend's code, adapted):
//
//  1. Far sensor (S1=Far Left, S5=Far Right) detected via stable
//     noise-filtered read → tight curve confirmed
//  2. Set turning=true and record turnDirection (-1=left, +1=right)
//  3. Creep forward briefly (110ms at baseSpeed) so the robot
//     body moves past the corner point before rotating
//  4. Enter turning mode: pivot on the spot every loop call
//     PID is completely OFF while turning=true
//  5. After a small delay (20ms) start checking ONLY S3 (center)
//  6. When S3 sees the line → turning=false → PID resumes
//
//  KEY DIFFERENCES from v6.5:
//  • No stopMotors() before pivot — creep forward first instead
//  • turning flag persists across loop() calls (not a blocking while)
//  • Only S3 exits the turn (not S2/S3/S4) — more precise
//  • Noise filter on far sensors prevents false corner triggers
// ============================================================

// Turning state (persistent across loop calls — same as friend's code)
bool  turning       = false;
...
void followLineAdaptive() {
  ...
  // ── READ FAR SENSORS with noise filter ──────────────────────
  int farLeft  = readFarSensorStable(IR_S1);   // S1 = Far Left
  int farRight = readFarSensorStable(IR_S5);   // S5 = Far Right

  // Check if any front sensor (S1–S5) sees the line
  bool frontDetect = (irBinary[0] || irBinary[1] || irBinary[2] ||
                      irBinary[3] || irBinary[4]);

  // ── TIGHT TURN TRIGGER ───────────────────────────────────────
  // Only trigger a new turn if not already turning.
  // Far LEFT sensor active → tight left curve → turn left
  if (!turning && frontDetect && farLeft == 1) {
    turning       = true;
    turnDirection = -1;          // LEFT
    turnStartTime = millis();
    integralSum   = 0.0;
    lastTightDir  = -1;
    Serial.println("TIGHT TURN DETECTED → LEFT");
    // Creep forward 110ms first — moves robot past the corner point
    driveMotors(BASE_SPEED, BASE_SPEED);
    delay(110);
  }

  // Far RIGHT sensor active → tight right curve → turn right
  if (!turning && frontDetect && farRight == 1) {
    turning       = true;
    turnDirection = 1;           // RIGHT
    turnStartTime = millis();
    integralSum   = 0.0;
    lastTightDir  = 1;
    Serial.println("TIGHT TURN DETECTED → RIGHT");
    driveMotors(BASE_SPEED, BASE_SPEED);
    delay(110);
  }

  // ── TURNING MODE ─────────────────────────────────────────────
  // While turning: PID is OFF, just pivot. Exit when S3 sees line.
  if (turning) {
    if (turnDirection == -1) {
    ...
    // Wait 20ms after turn start before checking S3
    // (avoids re-detecting the same line we just left)
    if (millis() - turnStartTime > 20) {
      readIRSensors();
      if (irBinary[2] == 1) {   // S3 center sees the line → done
        turning   = false;
        lastError = 0;
        Serial.println("TURN COMPLETE → resuming PID");
      }
    }
    return;  // skip PID entirely while turning
  }
  ...
  if (sensorSum == 0) {
    ...
    if (millis() - lineLostStart < 200) {
      // Short loss — keep last direction
