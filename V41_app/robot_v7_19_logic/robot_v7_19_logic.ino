/*
 * ============================================================
 * EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.19
...
// --- NEW TELEMETRY LIBRARIES ---
#include <WiFi.h>
#include <WebSocketsServer.h>
#include <ArduinoJson.h> // Ensure you have ArduinoJson version 6 installed
// -------------------------------
...
#define SHARP_TURN_SPEED      180   // pivot speed on tight corners
...
#define DETECT_DIST_CM       10   // 20 was first detection — start creeping toward object
#define COLOR_READ_DIST_CM    8   // stop and read color at this distance
...
const char* password = ""; // No password for easy testing
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  if(type == WStype_CONNECTED) {
    ...
    // --- NEW: PROCESS INCOMING COMMANDS FROM APP ---
    ...
    if (!error) {
      ...
      if (strcmp(cmd, "mode") == 0) {
        if (strcmp(val, "auto") == 0) {
          ...
          isAutoMode = true;
          ultrasonicEnabled = true; // Safety reset
          changeState(STATE_LINE_FOLLOW);
        } else {
          Serial.println(">>> SWITCHING TO MANUAL MODE");
          isAutoMode = false;
          stopMotors(); // Halt immediately
        }
      } 
        ...
        // Only accept move commands if we are in Manual Mode
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.19 — Full Logic + C2 Upgrade");
  Serial.println("============================================");
...
void loop() {
  ...
  if (millis() - lastTelemetryUpdate > 100) {
    StaticJsonDocument<200> doc;
    // Package live IR sensor array
    JsonArray irArray = doc.createNestedArray("irArray");
    for(int i=0; i<5; i++) {
      irArray.add(irBinary[i]);
    }
    
    // Package distance and current state
    doc["distanceCm"] = lastDistCm;
    
    // --- UPDATED: Show manual mode in dashboard ---
    if (isAutoMode) {
      doc["state"] = stateNames[currentState];
    } else {
      doc["state"] = "MANUAL OVERRIDE";
    }

    String jsonString;
    serializeJson(doc, jsonString);
    webSocket.broadcastTXT(jsonString); // Send to phone

    lastTelemetryUpdate = millis();
  }
  // ---------------------------

  // --- NEW: AUTO/MANUAL LOGIC SPLIT ---
  if (isAutoMode) {
    // Watchdog
    ...
    // --- MANUAL MODE ---
    // Update ultrasonic distance for the dashboard so it remains live
    lastDistCm = readUltrasonic();
    
    // Motor control logic is handled instantly inside webSocketEvent() 
    // to provide zero-latency remote driving.
  }
}
...
bool  centerMustClear   = false; // true when pivot triggered by 11100/00111
                                 // S3 is already ON at trigger, so we must
                                 // wait for S3 to go OFF then ON again.
                                 // Prevents immediate false pivot exit.
...
void followLineAdaptive() {
  ...
  if (turning) {
    ...
    // Guard: 30ms minimum before checking exit condition.
    if (millis() - turnStartTime > 30) {
      readIRSensors();

      // For 11100/00111: S3 was already ON when pivot started.
      // We must wait for S3 to go OFF (robot rotated away from line)
      // before we can accept S3 ON as a valid reacquire signal.
      if (centerMustClear && irBinary[2] == 0) {
        centerMustClear = false;
        // S3 cleared — now watch for reacquire
        Serial.println("S3 CLEARED — now watching for reacquire");
      }

      // EXIT when S3 center sees the line AND center has already cleared
      // (centerMustClear==false means either it was never set, or S3
      //  already went OFF and is now coming back ON — genuine reacquire)
  ...
  if (!turning && !reacquiring) {
    ...
    if (tightLeft || sharp90Left || missedLeft) {
      if (missedLeft) {
        ...
        delay(BACKUP_MS);
        stopMotors();
        centerMustClear = false;  // S3 was not ON at trigger
      } else {
        // covers 10000, 11110, 11100
        bool is11100 = (s1 && s2 && s3 && !s4 && !s5);
        centerMustClear = is11100;  // S3 is ON in 11100 — must clear first
        if (is11100) {
          Serial.println("TIGHT LEFT (11100) → pivot RIGHT — wait S3 clear first");
        } else {
          Serial.println("TIGHT LEFT (10000/11110) → pivot RIGHT");
        }
        stopMotors();
      }
      turning        = true;
      ...
      if (missedRight) {
        ...
        // covers 00001, 01111, 00111
        bool is00111 = (!s1 && !s2 && s3 && s4 && s5);
        centerMustClear = is00111;  // S3 is ON in 00111 — must clear first
        if (is00111) {
          Serial.println("TIGHT RIGHT (00111) → pivot LEFT — wait S3 clear first");
        } else {
          Serial.println("TIGHT RIGHT (00001/01111) → pivot LEFT");
        }
        stopMotors();
      }
      turning        = true;
...
void runTestMode() {
  ...
  if (TEST_COLOR) {
    ...
    while (true) {
      ...
      // reads and prints RAW, returns R/G/U
...
// ============================================================
//  END OF CODE v7.19 — EC6090 Mini Project 2026 — Full Logic
// ============================================================