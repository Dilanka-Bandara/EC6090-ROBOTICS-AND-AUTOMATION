/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.5
 *  CENTER EXIT — pivot until S3 (middle sensor) detects line
 * ============================================================
 ...
 *
 *  REQUEST:
 *    Keep turning until the MIDDLE sensor (S3, center) detects
 *    the line. When S3 detects → stop motors → resume PID.
 *    PID is working fine — do not change that part.
 *
 *  CHANGE 1 — PIVOT EXIT CONDITION:
 *    OLD: irBinary[1] || irBinary[2] || irBinary[3]
 *         (S2 OR S3 OR S4 — any inner sensor)
 *    NEW: irBinary[2] only
 *         (S3 CENTER only)
 *
 *    Why this is correct:
 *    S2 and S4 sit on the edges of the line. They fire when
 *    the robot is still slightly mis-aligned. Waiting for S3
 *    (the physical center sensor) means the robot keeps
 *    pivoting until the line is directly under the middle of
 *    the array — perfectly aligned before PID takes over.
 *
 *  CHANGE 2 — GUARD TIME:
 *    OLD: 200ms fixed minimum before checking exit
 *    NEW: 30ms minimum
 *
 *    The 200ms guard was the real problem — it was a fixed
 *    timer that made the robot pivot for at least 200ms
 *    regardless of where the line was. The pivot was stopping
 *    too early (200ms into a turn that needed 400ms) OR it
 *    was overshooting because the exit check started late.
 *
 *    30ms is just enough to prevent a false trigger from the
 *    entry sensor flash (the brief moment when S1/S5 is still
 *    fading as the pivot starts). After 30ms, the pivot checks
 *    S3 every single loop iteration (~1ms) — so the exit is
 *    as responsive as possible.
 *
 *  RESULT:
 *    The pivot now turns for EXACTLY as long as needed —
 *    no more, no less. Short 45° corners exit quickly.
 *    Full 90° corners turn all the way. The robot always
 *    stops with S3 centered on the line before PID starts.
 *
 *  ALL PREVIOUS FIXES KEPT:
 *    v7.4 — Pivot direction fix (sensors reversed)
 *    v7.3 — 3-phase smart corner (slow→pivot→reacquire)
 *    v7.2 — No delays in control path, 50Hz dashboard
 *    v7.1 — Missed corner detection, pivot timeout (slower corner entry)
 *
 ...
 *
 *  Author: EC6090 Mini Project 2026 — v7.5 Center Exit
 * ============================================================
 */
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.5 — Center Exit");
  Serial.println("============================================");
...
// ============================================================
//  END OF CODE v7.5 — EC6090 Mini Project 2026 — Center Exit
// ============================================================
