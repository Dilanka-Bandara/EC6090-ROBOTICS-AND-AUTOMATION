/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v6.5
 *  FIX — STOP BEFORE PIVOT ON TIGHT TURNS
 * ============================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v6.5 — Stop-Then-Pivot Tight Turns");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  if (sensorSum == 0) {
    ...
    // Short loss (<200ms): keep going in last known direction
    ...
    // Longer loss: spin using lastTightDir if available, else lastError
  ...
  // ── TIGHT TURN DETECTION (v6.5) ────────────────────────────
  //
  //  LOGIC:
  //    1. Far sensor (S1 or S5) fires → tight curve detected
  //    2. STOP motors immediately — kill forward momentum
  //    3. Short brake pause (50ms) to let robot settle
  //    4. Pivot on the spot toward the detected side
  //    5. Hold pivot until S2, S3, or S4 finds the line
  //    6. Return — normal PID takes over from here
  //
  //  lastTightDir tracks which far sensor last fired so if the
  //  robot loses the line mid-pivot it keeps turning the right way.
  ...
  // ── HARD LEFT: S1 active, right side clear ──────────────────
  if (s1 && !s4 && !s5) {
    integralSum   = 0.0;
    lastTightDir  = -1;   // remember: last tight turn was LEFT
    Serial.println("TIGHT TURN → STOP → HARD LEFT PIVOT");

    // STEP 1: Full stop — kill all momentum
    stopMotors();
    delay(60);            // 60ms brake pause — robot settles

    // STEP 2: Pivot LEFT on the spot
    //         LEFT wheel BACKWARD, RIGHT wheel FORWARD
    driveMotors(-SHARP_TURN_SPEED, SHARP_TURN_SPEED);

    // STEP 3: Hold pivot until a middle sensor finds the line
    unsigned long tStart = millis();
    while (millis() - tStart < 1200) {   // 1.2s safety timeout
      readIRSensors();
      if (irBinary[1] || irBinary[2] || irBinary[3]) break;
      delay(5);
    }

    // STEP 4: Stop pivot, let PID take over
    stopMotors();
    delay(30);
    lastError = -20;   // bias PID left so it doesn't overshoot right
    return;
  }

  // ── HARD RIGHT: S5 active, left side clear ──────────────────
  if (s5 && !s1 && !s2) {
    integralSum   = 0.0;
    lastTightDir  = 1;    // remember: last tight turn was RIGHT
    Serial.println("TIGHT TURN → STOP → HARD RIGHT PIVOT");

    // STEP 1: Full stop
    stopMotors();
    delay(60);

    // STEP 2: Pivot RIGHT on the spot
    //         LEFT wheel FORWARD, RIGHT wheel BACKWARD
    driveMotors(SHARP_TURN_SPEED, -SHARP_TURN_SPEED);

    // STEP 3: Hold pivot until middle sensor finds the line
    unsigned long tStart = millis();
    ...
    stopMotors();
    delay(30);
    lastError = 20;    // bias PID right so it doesn't overshoot left
    return;
  }
