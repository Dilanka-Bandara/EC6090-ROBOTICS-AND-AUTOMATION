/*
 * ============================================================
 *  EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v6.4
 *  FIX — SIMPLE FAR-SENSOR TIGHT TURN LOGIC
 * ============================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v6.4 — Simple Far-Sensor Turn Logic");
  Serial.println("============================================");
...
void followLineAdaptive() {
  ...
  // ── TIGHT TURN DETECTION (v6.4 logic) ──────────────────────
  //
  //  YOUR LOGIC: If FAR LEFT sensor (S1) is active → it is a tight
  //  curve to the left  → immediately do a hard LEFT pivot.
  //  If FAR RIGHT sensor (S5) is active → tight curve to the right
  //  → immediately do a hard RIGHT pivot.
  //
  //  No complex pattern needed. S1 active alone OR with others
  //  means the line has swung far left — hard turn NOW.
  //  Same for S5 on the right side.
  //
  //  S1 takes PRIORITY over S5 if both fire (shouldn't happen
  //  normally, but guards against noise).
  //
  //  Pivot direction:
  //    Hard LEFT  → LEFT wheel BACKWARD, RIGHT wheel FORWARD
  //                 driveMotors(-SHARP_TURN_SPEED, +SHARP_TURN_SPEED)
  //    Hard RIGHT → LEFT wheel FORWARD,  RIGHT wheel BACKWARD
  //                 driveMotors(+SHARP_TURN_SPEED, -SHARP_TURN_SPEED)
  //
  //  Hold the pivot inside a while-loop until S2, S3, or S4
  //  picks up the line — this means the robot has turned enough
  //  and the line is back under the middle sensors.
  ...
  // ── HARD LEFT: S1 is active (line is far left) ──────────────
  if (s1 && !s4 && !s5) {   // S1 fires AND right side is clear
    integralSum = 0.0;
    Serial.println("TIGHT TURN → HARD LEFT PIVOT");
    // LEFT wheel BACKWARD, RIGHT wheel FORWARD = pivot left
    driveMotors(-SHARP_TURN_SPEED, SHARP_TURN_SPEED);
    unsigned long tStart = millis();
    while (millis() - tStart < 1000) {   // 1s safety timeout
      readIRSensors();
      // Stop pivoting once S2, S3, or S4 sees the line
  ...
  // ── HARD RIGHT: S5 is active (line is far right) ────────────
  if (s5 && !s1 && !s2) {   // S5 fires AND left side is clear
    integralSum = 0.0;
    Serial.println("TIGHT TURN → HARD RIGHT PIVOT");
