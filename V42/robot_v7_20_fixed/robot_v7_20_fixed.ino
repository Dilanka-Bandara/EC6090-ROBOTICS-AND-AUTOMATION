/*
 * ============================================================
 * EC6090 MINI PROJECT 2026 — LINE FOLLOWING ROBOT v7.20
 * FULL LOGIC — OBSTACLE AVOID + REJOIN FIXED
 ...
 *
 * ============================================================
 * CHANGES FROM v7.19 → v7.20
 * ============================================================
 * FIX 1 — avoidObstacle() Step 2 no longer uses ultrasonic
 *   Old: while loop checking readUltrasonic() > 15cm
 *   Problem: ultrasonicEnabled=false here → always returns
 *   999cm → loop exits INSTANTLY → robot barely moves
 *   Fix: replaced with fixed delay(AVOID_PASS_MS) so robot
 *   actually drives past the obstacle before turning back
 *
 * FIX 2 — avoidObstacle() bypass direction now uses lastError
 *   Old: read IR after reversing to decide left/right
 *   Problem: on curved arena after reverse, robot not aligned
 *   so IR often sees nothing → always defaults to RIGHT
 *   Fix: use lastError captured at detection moment to decide
 *   direction (lastError > 0 = robot was drifting right =
 *   line is on left = go right to bypass, and vice versa)
 *
 * FIX 3 — rejoinLine() sweep speeds increased
 *   Old: driveMotors(90 +/- 30, ...) = too gentle, barely turns
 *   Fix: proper spin driveMotors(+/-110, -/+110) so robot
 *   actually sweeps wide enough to find the line
 *
 * ALL v7.19 LOGIC KEPT INTACT:
 * - WiFi AP + WebSocket telemetry
 * - Auto/Manual mode switching
 * - 8-state machine unchanged
 * - PID + corner phases (approach/pivot/reacquire)
 * - Color voting (15 samples)
 * - Close-approach to 5cm before color read
 * - Ultrasonic gate (ultrasonicEnabled flag)
 * - End zone active search (only when cubePickedUp)
 * - Watchdog recovery spin
 * - All test/calibration modes
 *
 ...
 *
 * Author: EC6090 Mini Project 2026 — v7.20 Avoid+Rejoin Fixed
 * ============================================================
 */
...
// ── v7.20 NEW: fixed time to drive past obstacle (Step 2)
// ultrasonicEnabled=false during avoidance so we CANNOT use
// distance. Use a fixed time instead. Tune 700–1000ms depending
// on your robot speed vs obstacle cube size (~10cm cube).
#define AVOID_PASS_MS       800
...
// ── v7.20: capture lastError at the moment obstacle is detected
// so avoidObstacle() can use it for direction even after the
// robot has moved (lastError may have changed by then).
...
void setup() {
  ...
  Serial.println();
  Serial.println("============================================");
  Serial.println("  EC6090 Robot v7.20 — Avoid+Rejoin Fixed");
  Serial.println("============================================");
    ...
    switch (currentState) {
      ...
      case STATE_LINE_FOLLOW: {
        ...
        if (dist > 2 && dist < DETECT_DIST_CM) {
          // Capture direction hint BEFORE stopping
          // lastError > 0 means robot drifted right → line is left → bypass right
          // lastError < 0 means robot drifted left  → line is right → bypass left
...
// ============================================================
//  LINE FOLLOWING — PID + Tight Turn + Missed Corner
// ============================================================
bool  turning           = false;
int   turnDirection     = 0;
...
// ============================================================
//  AVOID OBSTACLE  ← v7.20 FIXED
//
//  FIX 1: Step 2 now uses AVOID_PASS_MS fixed delay instead of
//         ultrasonic distance check (ultrasonic is disabled here
//         so it returns 999cm → old code exited step 2 instantly)
//
//  FIX 2: Bypass direction now uses avoidDirectionHint which was
//         captured from lastError at the moment of detection,
//         before any movement changed the robot's orientation.
//         avoidDirectionHint > 0 = go right, < 0 = go left.
// ============================================================
void avoidObstacle() {
  ...
  // ── FIX 2: Decide bypass direction from avoidDirectionHint ───
  // avoidDirectionHint was set in STATE_LINE_FOLLOW /
  // STATE_CARRY_TO_END the instant the object was detected,
  // before any movement happened.
  // +1 = robot was on/right of line → bypass RIGHT
  // -1 = robot was left of line     → bypass LEFT
  bool goRight = (avoidDirectionHint >= 0);
  if (goRight) Serial.println("  Direction: RIGHT bypass");
  else         Serial.println("  Direction: LEFT  bypass");
  int s = goRight ? 1 : -1;
  ...
  // ── Step 2: FIX 1 — drive forward for fixed time ─────────────
  // OLD code used ultrasonic which is DISABLED here (returns 999)
  // so the while loop exited instantly and the robot never moved.
  // NEW: fixed delay — robot drives forward long enough to pass
  // the cube. Tune AVOID_PASS_MS (default 800ms) if needed.
  Serial.println("  Step 2: Driving past obstacle (fixed time)...");
  driveMotors(AVOID_SPEED, AVOID_SPEED);
  delay(AVOID_PASS_MS);
  stopMotors(); delay(100);
...
// ============================================================
//  REJOIN LINE  ← v7.20 FIXED
//
//  FIX 3: Sweep speeds increased from 90±30 (barely drifts)
//         to proper spin 110/-110 so robot actually sweeps
//         wide enough to re-cross the yellow line.
// ============================================================
bool rejoinLine() {
  ...
  while (millis() - start < 5000) {
    ...
    // FIX 3: proper spin speeds (was 90±30 = too gentle)
    if (elapsed < 1500) {
      // First sweep in primary direction
      driveMotors( sweepDir * 110, -sweepDir * 110);
    } else if (elapsed < 3000) {
      // Reverse sweep in opposite direction
      driveMotors(-sweepDir * 110,  sweepDir * 110);
    } else {
      // Wide spin — last resort
      driveMotors( sweepDir * 130, -sweepDir * 130);
    }
    delay(30);
  }
...
// ============================================================
//  END OF CODE v7.20 — EC6090 Mini Project 2026
//  Fixes: avoidObstacle Step2 + bypass direction + rejoin speed
// ============================================================
